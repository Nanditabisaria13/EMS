...
const EmployeeLeaves = () => {
  ...
  const{backendUrl, token,getEmployeeLeaveData,leaveData} = useContext(EmployeeContext)
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
       try {
         const {data} = await axios.post(backendUrl + '/api/leave/apply-leave', leaveDetails, {headers:{token}} )
         if(data.success){
  ...
  return (
    ...
    <div className='w-full  sm:ml-[8.5rem] md:ml-[19.6rem]'>
      ...
      <div className='px-4 py-3 w-full bg-slate-50 dark:bg-[#101013]'>
        <h1 className='mt-1 font-medium text-2xl text-neutral-800 dark:text-white'>Leave Stats</h1>
        ...
        <button
          ...
          onClick={() => setShowModal(true)} >  Apply Leave    </button>
        </div>
      ...
      <div className="fixed inset-0 bg-gray-800 bg-opacity-50 flex justify-center items-center z-50">
        <div className="bg-white dark:bg-[#1a1a1a] p-6 rounded-lg w-96">
          ...
          <form onSubmit={handleSubmit}>
            {/* Leave Type */}
            <div className="mb-4">
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-100">Leave Type</label>
              <div className="flex gap-4 overflow-auto">
                ...
                <label className="inline-flex items-center">
                  <input
                    ...
                    checked={leaveDetails.leaveType === 'Paternity/Maternity'}
                    onChange={handleChange}
                    className="mr-2"
                  />
                  Paternity/Maternity
            ...
            {/* Reason */}
            <div className="mb-4">
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-100">Reason</label>
              <textarea
                name="reason"
            ...
            {/* Duration */}
            <div className="mb-4">
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-100">Duration</label>
              <input
                type="text"
                name="duration"
 ...
 <div  id='leaveStatus' className='bg-white drop-shadow-md p-5 mt-5 rounded relative border dark:bg-[#1a1a1a] 
                         dark:border-[#535353]  max-w-full '>

<h1 className='font-medium text-2xl text-neutral-800 dark:text-white'>Leave Details</h1>
...
<div className='overflow-x-auto'>
    ...
    <h5  className='text-lg font-normal w-32 lg:w-1/5 text-neutral-900-400'>{item.leaveType}</h5>
   <h3 className= 'text-lg font-normal w-32 lg:w-1/5 text-blue-400'>{item.duration}</h3>
   <h5  className='  text-lg font-normal w-32 lg:w-1/5 text-blue-500'>{item.startDate}</h5>
   <h5  className='  text-lg font-normal w-32 lg:w-1/5 text-neutral-800 dark:text-white '>{item.status}</h5>
