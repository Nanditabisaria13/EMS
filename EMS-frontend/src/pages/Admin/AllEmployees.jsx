...
const AllEmployees = () => {
  ...
  const{allEmployees,getAllEmployees,aToken,getSpecificEmployee,filterEmployees,filteredEmployees,searchResults} = useContext(AdminContext)
       ...
       <Sidebar/>
        <div className='w-full sm:ml-[10rem] md:ml-[19.6rem]'>
         ...
         <div className='p-4 relative flex flex-col gap-8'>
            <h1 className='font-medium text-2xl text-neutral-800 dark:text-white'>All Employees</h1>
          ...
          <div className='bg-emerald-700 text-white border-2 border-emerald-900 dark:bg-emerald-500 px-4 py-3 flex items-center gap-2
            rounded-lg '>
        ...
        {filterVisible && (
        <div className="absolute top-36 left-20 bg-white dark:bg-[#1a1a1a] dark:border-[#535353] dark:text-white shadow-md border rounded-lg w-48
          p-2 z-10">
          <h1 className='text-lg font-normal p-2 cursor-pointer'>Filter by:</h1>
          <div className="p-2 cursor-pointer hover:bg-gray-200 dark:hover:bg-gray-400" onClick={() => handleFilterSelect('active')}>
             Active Tasks</div>
          <div className="p-2 cursor-pointer hover:bg-gray-200 dark:hover:bg-gray-400" onClick={() => handleFilterSelect('completed')}>
            Completed Tasks </div>
          <div className="p-2 cursor-pointer hover:bg-gray-200 dark:hover:bg-gray-400" onClick={() => handleFilterSelect('failed')}>
            Failed Tasks </div>
        </div>
      )}
...
{  
   allEmployees?(
  searchResults.length > 0 ? searchResults.map((employee, index) => (
    <div key={index} className='flex items-start justify-between p-4 gap-2 bg-white drop-shadow-md rounded-xl border
    border-zinc-300 transition-all hover:scale-105 duration-500 dark:bg-[#1a1a1a] dark:border-[#535353]'>
      <img src={employee.image} className='w-20 h-20 rounded-full' alt="" />
      <div className='flex flex-col'>
        ...
        <p className='text-lg md:text-sm lg:text-base font-normal text-gray-600 dark:text-gray-100'>{employee.position}</p>
      </div>
